...
def split_by_low_regions(arr: np.ndarray, threshold: int = 128) -> List[Tuple]:
    """
...
def remove_bottom_arc(pixel_array: np.ndarray, boundary: list):
    """
    CT 이미지 하단의 아치형 구조를 제거하는 함수

    Args:
        pixel_array (np.ndarray): CT 이미지 배열

    Returns:
        np.ndarray: 아치형 구조가 제거된 CT 이미지 배열
    """
    h, w = pixel_array.shape
    for x in range(w):
...
def check_sternum(pixel_array: np.ndarray):
    """
    ...
    for hu_range in bone_hu_value:
        count += np.sum((target_area >= hu_range[0]) & (target_area <= hu_range[1]))
    return count > 64

def find_heart(pixel_array: np.ndarray):
    """
    CT 이미지에서 심장을 찾는 함수
    ...
    heart_mask = np.zeros_like(pixel_array, dtype=np.uint8)
    heart_mask[(pixel_array >= hu[0]) & (pixel_array <= hu[1])] = 1
    reversed_heart_mask = np.zeros_like(pixel_array, dtype=np.uint8)
...
def masking_heart(pixel_array: np.ndarray, normalized_pixel_array: np.ndarray, heart_mask: np.ndarray):
    """
    심장을 마스킹하는 함수

    Args:
        pixel_array (np.ndarray): 이미지 배열
        normalized_pixel_array (np.ndarray): 정규화된 이미지 배열
        heart_mask (np.ndarray): 심장 마스크

    Returns:
        np.ndarray: 심장이 마스킹된 이미지 배열
        np.ndarray: 심장이 마스킹된 정규화된 이미지 배열
    """
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
...
def find_liver(pixel_array: np.ndarray):
    """
    CT 이미지에서 간을 찾는 함수
    ...
    liver_mask = np.zeros_like(pixel_array, dtype=np.uint8)
    liver_mask[(pixel_array >= hu[0]) & (pixel_array <= hu[1])] = 1
    reversed_liver_mask = np.zeros_like(pixel_array, dtype=np.uint8)
...
def masking_liver(pixel_array: np.ndarray, normalized_pixel_array: np.ndarray, liver_mask: np.ndarray):
    """
    간을 마스킹하는 함수
    ...
    Args:
        pixel_array (np.ndarray): 이미지 배열
        normalized_pixel_array (np.ndarray): 정규화된 이미지 배열
        liver_mask (np.ndarray): 간 마스크

    Returns:
        np.ndarray: 간이 마스킹된 이미지 배열
        np.ndarray: 간이 마스킹된 정규화된 이미지 배열
    """
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
...
def find_contours(pixel_array: np.ndarray):
    """
    ...
    binary_image = np.zeros_like(gray_pixel_array)
...
def fill_contours(pixel_array: np.ndarray, contours: list):
    """
    이미지 윤곽선을 채우는 함수
...
def remove_skin(pixel_array: np.ndarray, contours: list, method: str, thickness: int = 16):
    """
    이미지에서 피부를 제거하는 함수
    ...
    try:
        ...
        # 폴리곤 채우기
        try:
            cv2.fillPoly(pixel_array, [np.array(bottom_points, dtype=np.int32)], 0)
        except Exception as e:
...
def find_muscle(pixel_array: np.ndarray, is_lung: bool):
    """
    ...
    """
    if is_lung:
        muscle_hu_value = [[980, 1200]]
    else:
        muscle_hu_value = [[980, 1200]]
    muscle = muscle_hu_value[0]
    muscle_mask = np.zeros(pixel_array.shape)
    ...
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(muscle_mask.astype(np.uint8), connectivity=8)
    back_skin_idx = -1
    flag = True
...
def fill_muscle(pixel_array: np.ndarray, muscle_mask: np.ndarray):
    """
    근육을 채우는 함수

    Args:
        pixel_array (np.ndarray): 이미지 배열
        muscle_mask (np.ndarray): 근육 마스크

    Returns:
        np.ndarray: 근육이 채워진 이미지 배열
    """
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
...
def find_bone(pixel_array: np.ndarray, muscle_mask: np.ndarray):
    """
    HU 값으로 뼈를 찾는 함수
    ...
    bone_mask = np.zeros(pixel_array.shape)
    for i in range(pixel_array.shape[0]):
        for j in range(pixel_array.shape[1]):
            if cortical_bone[0] <= pixel_array[i, j] <= cortical_bone[1]:
                bone_mask[i, j] = 1
    num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(bone_mask.astype(np.uint8), connectivity=8)
    for i in range(1, num_labels):
        if stats[i, cv2.CC_STAT_AREA] < 64:
            bone_mask[labels == i] = 0
    contours, _ = cv2.findContours(bone_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contour in contours:
        ...
        if contour_center[0][1] < 200:
            ...
            contour_pixels = np.zeros_like(cur_pixels)
            cur_pixels[contour_pixels == 0] = 0
            active_pixel_count = np.sum(cur_pixels)
...
def masking_bone(pixel_array: np.ndarray, bone_mask: np.ndarray, bone_contours: list = None):
    """
    뼈를 마스킹하는 함수

    Args:
        pixel_array (np.ndarray): 이미지 배열
        bone_mask (np.ndarray): 뼈 마스크

    Returns:
        np.ndarray: 뼈가 마스킹된 이미지 배열
    """
...
def masking_bone_inner_area(pixel_array: np.ndarray, bone_inner_points: np.ndarray):
    """
    갈비뼈 및 척추를 기준으로 내부를 마스킹하는 함수

    Args:
        pixel_array (np.ndarray): 이미지 배열
        bone_inner_points (np.ndarray): 내부 윤곽선 포인트

    Returns:
        np.ndarray: 내부가 마스킹된 이미지 배열
    """
    if len(bone_inner_points) > 2:
        try:
            ...
            bone_center_point = np.mean(red_coords, axis=0).astype(int)
            
            # 안전한 거리 계산을 위한 포인트 변환
            try:
                bone_points_2d = np.array(bone_inner_points)[:, 0, :]  # (N, 2) 형태로 변환
                bone_center_2d = np.array([bone_center_point[1], bone_center_point[0]])  # (y,x) -> (x,y)
                
                distances = np.linalg.norm(bone_points_2d - bone_center_2d, axis=1)
                ...
                if len(target_bone_inner_points) > 2:
                    try:
                        hull = ConvexHull(np.array(target_bone_inner_points)[:, 0, :])
                        hull_center = np.mean(np.array(target_bone_inner_points)[hull.vertices], axis=0)
                        hull_center = hull_center * 0.9 + np.mean(hull_center, axis=0) * 0.1
                        mask_convex = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
...
def find_vertebrae_contour(bone_contours: list):
    ...
    Args:
        bone_contours (np.ndarray): 뼈 윤곽선
    ...
    vertebrea_contour = np.array(quadrant[0] + quadrant[1] + quadrant[2] + quadrant[3])
    return np.array(vertebrea_contour)
...
def find_bone_inner_contours(bone_contours: list):
    ...
    Returns:
        np.ndarray: 몸 중심을 바라보는 윤곽선 포인트들
    """
    ...
    for bone_contour in bone_contours:
        ...
        if contour_size > 64:
            ...
            closest_indices = np.argsort(distances)[:len(distances)//4]
    ...
    quadrant[2] = sorted(quadrant[2], key=lambda x: x[0][0]) # quadrant 3
    quadrant[3] = sorted(quadrant[3], key=lambda x: x[0][0]) # quadrant 4
    bone_inner_points = np.array(quadrant[0] + quadrant[1] + quadrant[2] + quadrant[3])
    return bone_inner_points

def masking_vertebrae(pixel_array: np.ndarray, vertebrae_contour: np.ndarray):
    """
    척추뼈와 척수를 마스킹하는 함수

    Args:
        pixel_array (np.ndarray): 이미지 배열
        vertebrae_contour (np.ndarray): 척추 윤곽선

    Returns:
        np.ndarray: 척추뼈와 척수가 마스킹된 이미지 배열
    """
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
...
def masking_vertebrae_muscle(normalized_pixel_array: np.ndarray, vertebrae_contour: np.ndarray):
    """
    척추 윤곽선을 기준으로 근육 영역을 마스킹하는 함수

    Args:
        normalized_pixel_array (np.ndarray): 정규화된 이미지 배열
        vertebrae_contour (np.ndarray): 척추 윤곽선 배열

    Returns:
        np.ndarray: 마스킹된 이미지 배열
    """
    red_pixels = (normalized_pixel_array[:, :, 0] == 255) & (normalized_pixel_array[:, :, 1] == 0) & (normalized_pixel_array[:, :, 2] == 0)
    
    body_left_x = np.min(np.where(red_pixels)[1])
    body_right_x = np.max(np.where(red_pixels)[1])
    body_width = body_right_x - body_left_x
    body_left_q1_point = body_left_x + body_width // 4
    body_right_q1_point = body_right_x - body_width // 4                                                    

    vertebrae_left_x = np.min(vertebrae_contour[:, 0, 0])
    vertebrae_right_x = np.max(vertebrae_contour[:, 0, 0])
    vertebrae_top_y = np.min(vertebrae_contour[:, 0, 1])
    vertebrae_width = vertebrae_right_x - vertebrae_left_x
    left_q1_point = vertebrae_left_x + vertebrae_width // 4
    right_q1_point = vertebrae_right_x - vertebrae_width // 4
    
    left_q1_points = vertebrae_contour[(vertebrae_contour[:, 0, 0] >= vertebrae_left_x) & 
                    (vertebrae_contour[:, 0, 0] <= left_q1_point)]
    if len(left_q1_points) > 0:
        left_base_point = left_q1_points[np.argmax(left_q1_points[:, 0, 1])][0]
    else:
        left_base_point = (vertebrae_left_x, 0)
        
    right_q1_points = vertebrae_contour[(vertebrae_contour[:, 0, 0] <= vertebrae_right_x) &
                        (vertebrae_contour[:, 0, 0] >= right_q1_point)]
    if len(right_q1_points) > 0:
        right_base_point = right_q1_points[np.argmax(right_q1_points[:, 0, 1])][0]
    else:
        right_base_point = (vertebrae_right_x, 0)
        
    base_y = max(left_base_point[1], right_base_point[1])
        
    points = [
        (body_right_q1_point, vertebrae_top_y), 
        (body_right_q1_point, base_y - 40),
        ((body_right_q1_point + right_base_point[0]) // 2, base_y - 32),
        (right_base_point[0], base_y),
        (left_base_point[0], base_y), 
        ((body_left_q1_point + left_base_point[0]) // 2, base_y - 32), 
        (body_left_q1_point, base_y - 40), 
        (body_left_q1_point, vertebrae_top_y), 
    ]
    
    polygon = np.array(points, dtype=np.int32)

    cv2.fillPoly(normalized_pixel_array, [polygon], (0, 0, 0))
    return normalized_pixel_array

def masking_breast(pixel_array: np.ndarray, bone_contours: list):
    """
    유방을 마스킹하는 함수
    ...
    if np.sum(roi == 255) > 0:
        top_y = np.where(temp_pixels[:, 224:308, 0] == 255)[0].min()
    else:
        top_y = h // 2 - 32
    edge_points = np.array([[(0, h)], [(0, h // 2 + 32)], [(w//2, top_y - 8)],[(w, h // 2 + 32)], [(w, h)]])
    all_points = np.concatenate(bone_contours)
    all_points = np.concatenate([all_points, edge_points])
    hull = ConvexHull(all_points[:, 0, :])
    
    scaled_hull_points = (all_points[hull.vertices] - np.mean(all_points[hull.vertices], axis=0)) * 1.0875 + np.mean(all_points[hull.vertices], axis=0)
    ...
    for x in range(0, temp_pixels.shape[1], 64):
        ...
        y_values = np.where(yellow_pixels[:, x])[0]
        if len(y_values) > 0:
            min_y = y_values.min()
    ...
    if len(hull_top_points) > 2:
        hull_top_points = np.array(hull_top_points)
        x = hull_top_points[:, 0]
        y = hull_top_points[:, 1]
        coefficients = np.polyfit(x, y, 2)  # 2차 함수 계수 계산
    ...
    black_polygon_points = np.concatenate([np.argwhere(blue_pixels), [(w, h), (0, h),]])
    cv2.fillPoly(temp_pixels, [black_polygon_points], (0, 0, 0))
    
    breast_mask = np.zeros(temp_pixels.shape[:2], dtype=np.uint8)
...
def find_abs(norm_idx: float, pixel_array: np.ndarray, bone_contours: list, vertebrae_contour: list):
    """
    ...
    try:
        ...
        if len(vertebrae_contour) > 0:
            try:
                vertebrae_center = np.mean(vertebrae_contour, axis=0).astype(int)
...
def masking_inner_area_from_over_lung(pixel_array: np.ndarray, contours: list, bone_contours: list):
    """
    전체 영역 중 폐 상단 영역에서 내부를 마스킹하는 함수

    Args:
        pixel_array (np.ndarray): 이미지 배열
        contours (np.ndarray): 윤곽선 배열
        bone_contours (np.ndarray): 뼈 윤곽선 배열

    Returns:
        np.ndarray: 내부가 마스킹된 이미지
    """
    h, w = pixel_array.shape[:2]
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
    ...
    if len(bone_contours) > 7:
        ...
        for contour in bone_contours:
            if not np.array_equal(contour, left_bone_1) and not np.array_equal(contour, left_bone_2) and not np.array_equal(contour, right_bone_1) and not np.array_equal(contour, right_bone_2):
                temp_contours.append(contour)
    else:
        left_bone_1 = sorted_left_bone_contours[0]
        right_bone_1 = sorted_right_bone_contours[0]
        for contour in bone_contours:
            if not np.array_equal(contour, left_bone_1) and not np.array_equal(contour, right_bone_1):
                temp_contours.append(contour)
    ...
    if len(bone_contours) > 0:
        all_points = np.concatenate(bone_contours)
        convex_points = []
        center_point = np.mean(all_points, axis=0, dtype=int)
        ...
        if convex_points:
            convex_points = np.array(convex_points, dtype=np.int32).reshape((-1, 1, 2))
            cv2.drawContours(pixel_array, [convex_points], -1, (0, 0, 0), -1)
    return pixel_array

def masking_inner_area_from_lung(pixel_array: np.ndarray, contours: list, bone_contours: list, vertebrae_contour: np.ndarray):
    """
    ...
    h, w = pixel_array.shape[:2]
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
    bone_points = np.concatenate(bone_contours)
    ...
    boundary_points = np.argwhere(mask == 1)
    ...
    sorted_boundary_points = np.array(quadrants[0] + quadrants[1] + quadrants[2] + quadrants[3])
    ...
    cv2.drawContours(pixel_array, [np.array(bone_inner_points)], -1, (0, 0, 0), -1)
    if len(contours) >= 3:
        target_lung_contour = contours[1:]
        target_points = np.concatenate(target_lung_contour)
...
def masking_inner_area_from_under_lung(pixel_array: np.ndarray, contours: list, bone_contours: list):
    """
    ...
    red_pixels = (pixel_array[:, :, 0] == 255) & (pixel_array[:, :, 1] == 0) & (pixel_array[:, :, 2] == 0)
    x, y, w, h = cv2.boundingRect(red_pixels.astype(np.uint8))
    xx, yy, ww, hh = cv2.boundingRect(red_pixels.astype(np.uint8))
        ...
        for r in range(1, max(pixel_array.shape)):
            ...
            if 0 <= x < pixel_array.shape[0] and 0 <= y < pixel_array.shape[1]:
                ...
                if red_pixels[x, y]:
                    ...
                    break
    try:
        distance = np.mean(np.linalg.norm(np.array(found_points) - center_point, axis=1))
        dispersion = np.std(np.linalg.norm(np.array(found_points) - center_point, axis=1))
        distance = distance - dispersion // 5 * 2
        filtered_points = [point for point in found_points if np.linalg.norm(point - center_point) > distance]
        filtered_points = np.array(filtered_points)
        filtered_under_vertebrae_points = [point for point in under_vertebrae_points if 206 <= point[0] <= 306]
        filtered_under_vertebrae_points = np.array(filtered_under_vertebrae_points)
        if len(filtered_under_vertebrae_points) == 0:
            all_points = filtered_points
        else:
            all_points = np.concatenate([filtered_points, filtered_under_vertebrae_points])
        ...
        vertebrae_left_bottom_points = [point for point in np.argwhere(green_pixels) if point[1] < center_point[0] - 16 and point[0] > center_point[1] + 32]
        vertebrae_right_bottom_points = [point for point in np.argwhere(green_pixels) if point[1] > center_point[0] + 16 and point[0] > center_point[1] + 32]
        ...
        for i in range(len(points) - 1):
            ...
            pos1 = points[i]
            pos2 = points[i + 1]
            distance_pos = np.linalg.norm(np.array(pos1) - np.array(pos2))
        ...
        cv2.fillPoly(pixel_array, [np.array(sorted_points)], (0, 0, 0))
        cv2.fillPoly(pixel_array, [np.array(outline_points)], (0, 0, 0))
        
        red_contours, _ = cv2.findContours(red_pixels.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if red_contours:
            all_red_points = np.concatenate(red_contours)
            hull = ConvexHull(all_red_points[:, 0, :])
            hull_area = all_red_points[hull.vertices]
        # outline = contours[0]
        outline = hull_area
        outline_center_point = np.mean(outline, axis=0, dtype=np.float32)
        ...
        outline = outline.astype(np.int32)
        outline = np.array([point for point in outline if point[0][1] <= center_point[0]])
        if outline is not None and len(outline) > 0:
            outline = outline.astype(np.int32).reshape((-1, 1, 2))
            cv2.drawContours(pixel_array, [outline], -1, (0, 0, 0), -1)    
        center_points = np.array([center_left_top_point, center_left_bottom_point, center_right_bottom_point, center_right_top_point])
        cv2.fillPoly(pixel_array, [center_points], (0, 0, 0))
        
        # 검은 픽셀에 대한 마스크 생성
        black_mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
        black_pixels = (pixel_array[:, :, 0] < 16) & (pixel_array[:, :, 1] < 16) & (pixel_array[:, :, 2] < 16)
        black_mask[black_pixels] = 255

        # 검은 픽셀 마스크의 컨투어 탐색
        black_contours, _ = cv2.findContours(black_mask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        black_contours = [contour for contour in black_contours if cv2.contourArea(contour) > 100]
        black_contours = sorted(black_contours, key=cv2.contourArea, reverse=True)
        
        inner_area_contour = black_contours[2]
        inner_area_contour = [point for point in black_contours[2] if center_point[0] - 32 <= point[0][1] <= center_point[0] + 32]
        inner_area_contour = np.array(inner_area_contour)
        inner_area_hull = ConvexHull(inner_area_contour[:, 0, :])
        inner_area = inner_area_contour[inner_area_hull.vertices]
        
        cv2.fillPoly(pixel_array, [inner_area], (0, 0, 0))

    except:
        pass
    return pixel_array

def extract_muscle_only(pixel_array: np.ndarray, remove_noise: bool = True):
    """
    근육 영역만 추출하는 함수
    ...
    Returns:
        np.ndarray: 근육 영역만 추출된 이미
    """
    red_pixels = (pixel_array[:, :, 0] == 255) & (pixel_array[:, :, 1] == 0) & (pixel_array[:, :, 2] == 0)
    black_pixels = (red_pixels[:, :] == 0)
    mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
    reversed_mask = np.zeros(pixel_array.shape[:2], dtype=np.uint8)
    mask[red_pixels] = 255
...
def preprocess(idx: int, category: str, sn: str, dicom_path: str, visualize_dir: str, boundary: list):
    ...
    Returns:
        str: 시각화된 이미지 경로
        np.ndarray: CT 이미지 배열
        np.ndarray: 정규화된 CT 이미지 배열
    """
...
def extract_muscle_from_over_lung(
        ...
        normalized_pixel_array: np.ndarray
    ):
    """
    ...
    Args:
        ...
        pixel_array (np.ndarray): 원본 이미지 배열
        normalized_pixel_array (np.ndarray): 정규화된 이미지 배열
...
def extract_muscle_from_under_lung(
    ...
    normalized_pixel_array = masking_bone_inner_area(normalized_pixel_array, bone_inner_points)
    normalized_pixel_array = masking_vertebrae(normalized_pixel_array, vertebrae_contour)
    normalized_pixel_array = masking_vertebrae_muscle(normalized_pixel_array, vertebrae_contour)
